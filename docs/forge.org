...
#+texinfo: @noindent
This manual is for Forge version 0.6.9.
...
please first setup the latter.  The setup for https://github.com is
easier, and if that works but the setup for the other Github instance
fails, then we can tentatively narrow the issue down to the parts that
differ between https://github.com and other instances.
...
While Forge knows about https://github.com out of the box, for all
other instances you have to add a new entry to option ~forge-alist~.

The entry for https://github.com looks like this:
...
your company uses https://example.com, this might be correct:
...
please first setup the latter.  The setup for https://gitlab.com is
easier, and if that works but the setup for the other Gitlab instance
fails, then we can tentatively narrow the issue down to the parts that
differ between https://gitlab.com and other instances.
...
instances) out of the box, for other instances you have to add a new
entry to option ~forge-alist~.

The entry for https://gitlab.com looks like this:
...
You have to add an entry for your instance.  For example, assuming
your company/organisation uses https://example.com, this might be
correct:
...
- Key: N / a (forge-add-repository) ::
  ...
  contribute to that repository.  You can later switch to fetching
  all topics, using ~forge-complete-repository~.
...
- Key: N m c (forge-complete-repository) ::
...
- Key: RET [on topic] (forge-visit-this-topic) ::
  ...
  a separate buffer, it at the same time displays the topic menu.
...
- Key: N v u (forge-visit-topic-from-url) ::
  ...
  If the repository, which the topic belongs to, isn't being tracked
  in the local database, then it is added automatically, but without
  fetching all topics.  You can later start fetching all topics using
  the command ~forge-complete-repository~.
...
- Key: C-<return> [on a topic section] ::
  ...
  With a prefix argument, this additionally visits the topic.
...
- User Option: forge-browse-topics-using-forge ::

  This option controls whether topic URLs are visited using Forge.
...
** Forge repositories in database vs. Git repositories

Forge differs from Emacs packages like Magit and ~project~, in that its
first-class objects are remote repositories, not local file system
paths.  Forge "knows" about a set of remote repositories, which can
be listed with ~M-x forge-list-repositories~.  Forge adds a remote
repository to its known/tracked set upon explicit request, typically
~N f f~ from Magit.

A repository may be optionally associated with a local git worktree,
but this is not exclusive.  It is possible to use the same forge
repository with multiple worktrees.  The last used worktree will be
remembered as the associated one.

Mapping from ~default-directory~ to a Forge repository is supported,
according to fixed rules (see [[*How Forge Detection Works]]).  In short,
if there is only one remote Forge uses that; otherwise it tries the
value of ~forge.remote~, then a remote named ~upstream~, if that exists,
and finally a remote named ~origin~, if that exists.

Forge stores its state in a central database (~forge-database-file~).
The database is largely cache-like, but not entirely.  At least the
following information cannot be re-fetched if the database is deleted:

- Choice of repositories, individual topics (in selective workflow)
- Choice of selective mode, or not having fetched topics before some date
- Associated worktree path
- Private per-topic status (unread, pending, done), topic marks (and
  mark definitions), notes
